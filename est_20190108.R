...
# Select smaller subsample to decrese computation time
n_obs <- 10000
df <- data_raw %>%
...
preProcValues <- preProcess(baseline_covariates_cont_obs, method = c("center", "scale"))
ObsTransformed <- predict(preProcValues, baseline_covariates_cont_obs)
...
lasso.linear <- cv.glmnet(lasso_covariates_train, final_price_train,lambda.min = exp(-9),  alpha=1, parallel=TRUE)
plot(lasso.linear)
