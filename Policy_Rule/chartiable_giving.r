...
#y1_hat_obs <- f_y1_obs$predictions
#f_y1_obs <- predict(f_y1_A, covariates_obs)
#f_y1_hold_out <- predict(f_y1_A, covariates_hold_out)
...
########################  Plot Distribution of Griving Probability  ########################

d2 <- density(gprob)
plot(d2, main="Distribution of Predicted CATEs",  xlab = "Charitable Giving Probability")
...
print(paste0("Optimal number final leaves: ", op.index_2_A))
op.index_2_B <- which.min(tree_2_B$cptable[, "xerror"])
print(paste0("Optimal number final leaves: ", op.index_2_B))
...
rule_gprob <- as.numeric(gprob_hold_out < mean(gprob_hold_out))
...
describe(cbind(rule_gprob,rule_cate,rule_tree_1,rule_tree_2))

print('Correlation between the Policy Rules')
cor(cbind(rule_gprob,rule_cate,rule_tree_1,rule_tree_2))
...
print('Total Regret as Opposed to Random Assignment')
...
R2_gprob <- round(mean(rule_gprob)*(mean(giving_hold_out[treat_hold_out == 1 & rule_gprob == 1])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_gprob == 1]))
             - mean(1-rule_gprob)*(mean(giving_hold_out[treat_hold_out == 1 & rule_gprob == 0])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_gprob == 0])), digits = 2)

R2_cate <- round(mean(rule_cate)*(mean(giving_hold_out[treat_hold_out == 1 & rule_cate == 1])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_cate == 1]))
             - mean(1-rule_cate)*(mean(giving_hold_out[treat_hold_out == 1 & rule_cate == 0])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_cate == 0])), digits = 2)

R2_tree_1 <- round(mean(rule_tree_1)*(mean(giving_hold_out[treat_hold_out == 1 & rule_tree_1 == 1])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_tree_1 == 1]))
             - mean(1-rule_tree_1)*(mean(giving_hold_out[treat_hold_out == 1 & rule_tree_1 == 0])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_tree_1 == 0])), digits = 2)

R2_tree_2 <- round(mean(rule_tree_2)*(mean(giving_hold_out[treat_hold_out == 1 & rule_tree_2 == 1])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_tree_2 == 1]))
             - mean(1-rule_tree_2)*(mean(giving_hold_out[treat_hold_out == 1 & rule_tree_2 == 0])
            - mean(giving_hold_out[treat_hold_out == 0 & rule_tree_2 == 0])), digits = 2)


print('Total Regret as Opposed to Opposite Treatment Assignment')
print(paste0("Probability Rule: ", R2_gprob*nrow(df)))
print(paste0("CATE Rule: ", R2_cate*nrow(df)))
print(paste0("Tree 1 Rule: ", R2_tree_1*nrow(df)))
print(paste0("Tree 2 Rule: ", R2_tree_2*nrow(df)))
